...
tests("UNIT: gaugeChart -", function(t) {
    let _chart = null;
    let _model = null;
    ...
    t.methods(type, [
        ...
        "legend", //module
        "controls", //module
        "setPointer", //not testable
    ]);

    t.beforeEach(function (assert) {
        _chart = sucrose.charts.gaugeChart();
        _model = _chart.gauge;
        assert.end();
    });
    ...
    t.test("x: ", function(assert) {
        ...
        assert.equal(_chart.x()({key: 1}), 1, "returns value if value is defined");
    ...
    t.test("showPointer: ", function(assert) {
        assert.plan(2);
        let def = true;
        assert.equal(_chart.showPointer(), def, "returns default value");
        let val = false;
        _chart.showPointer(val);
        assert.equal(_chart.showPointer(), val, "returns set value");
        t.register(assert, type);
    });
    //TODO: move to integraton test
    // t.test("setPointer: ", function(assert) {
    //     assert.plan(2);
    //     let def = _chart.setPointer();
    //     assert.equal(typeof def === "function", def, "returns default function");
    //     let val = function(){return 100;};
    //     _chart.setPointer(val);
    //     assert.equal(_chart.setPointer(), val, "returns set value");
    //     t.register(assert, type);
    // });
    t.test("ringWidth: ", function(assert) {
        assert.plan(2);
        let def = 50;
        assert.equal(_chart.ringWidth(), def, "returns default value");
        let val = 75;
        _chart.ringWidth(val);
        assert.equal(_chart.ringWidth(), val, "returns set value");
        t.register(assert, type);
    });
    ...
    t.test("maxValue: ", function(assert) {
        ...
        let val = 1;
        _chart.maxValue(val);
        assert.equal(_chart.maxValue(), val, "returns set value");
        t.register(assert, type);
    });
    t.test("minValue: ", function(assert) {
        assert.plan(2);
        let def = 0;
        assert.equal(_chart.minValue(), def, "returns default value");
        let val = 1;
        _chart.minValue(val);
        assert.equal(_chart.minValue(), val, "returns set value");
        t.register(assert, type);
    });
    t.skip("setPointer: ", function(assert) {
        assert.plan(2);
        let def = _chart.setPointer();
        assert.equal(typeof def, "function", "returns default function");
        let val = function(){return 1;};
        _chart.setPointer(val);
        assert.equal(_chart.setPointer(), val, "returns set function");
        t.register(assert, type);
    });
