...
tests("UNIT: areaChart -", function(t) {
    let _chart = null;
    let _model = null;
    ...
    t.beforeEach(function (assert) {
        _chart = sucrose.charts.areaChart();
        _model = _chart.area;
        assert.end();
    });
    ...
    t.test("offset: ", function(assert) {
        assert.plan(2);
        let def = "zero";
        assert.equal(_chart.offset(), def, "returns default value");
        let val = "wiggle";
        _chart.offset(val);
        assert.equal(_chart.offset(), val, "returns set value");
        t.register(assert, type);
    });
    t.test("order: ", function(assert) {
        assert.plan(2);
        let def = "default";
        assert.equal(_chart.order(), def, "returns default value");
        let val = "inside-out";
        _chart.order(val);
        assert.equal(_chart.order(), val, "returns set value");
        t.register(assert, type);
    });
    t.test("style: ", function(assert) {
        assert.plan(2);
        let def = "stack";
        assert.equal(_chart.style(), def, "returns default value");
        let val = "stream";
        _chart.style(val);
        assert.equal(_chart.style(), val, "returns set value");
        t.register(assert, type);
    });
