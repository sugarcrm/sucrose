...
tests("UNIT: multibarChart -", function(t) {
    let _chart = null;
    let _model = null;
    ...
    t.beforeEach(function (assert) {
        _chart = sucrose.charts.multibarChart();
    ...
    t.test("vertical: ", function(assert) {
        assert.plan(2);
        let def = true;
        assert.equal(_chart.vertical(), def, "returns default value");
        let val = false;
        _chart.vertical(val);
        assert.equal(_chart.vertical(), val, "returns set value");
        t.register(assert, type);
    });
    t.test("allowScroll: ", function(assert) {
        assert.plan(2);
        let def = true;
        assert.equal(_chart.allowScroll(), def, "returns default value");
        let val = false;
        _chart.allowScroll(val);
        assert.equal(_chart.allowScroll(), val, "returns set value");
        t.register(assert, type);
    });
    t.test("hideEmptyGroups: ", function(assert) {
        assert.plan(2);
        let def = true;
        assert.equal(_chart.hideEmptyGroups(), def, "returns default value");
        let val = false;
        _chart.hideEmptyGroups(val);
        assert.equal(_chart.hideEmptyGroups(), val, "returns set value");
        t.register(assert, type);
    });
    ...
    t.test("overflowHandler: ", function(assert) {
        assert.plan(2);
        var def = _chart.overflowHandler();
        assert.equal(typeof def, "function", "returns default function");
        let val = function(){return "asdf";};
        _chart.overflowHandler(val);
        assert.equal(_chart.overflowHandler(), val, "returns set function");
        t.register(assert, type);
    });
