...
tests("UNIT: lineChart -", function(t) {
    let _chart = null;
    let _model = null;
    ...
    t.beforeEach(function (assert) {
        _chart = sucrose.charts.lineChart();
        _model = _chart.lines;
        assert.end();
    });
    ...
    t.test("y: ", function(assert) {
        ...
        var def = _chart.y();
        assert.equal(typeof def, "function", "returns default function");
        assert.equal(typeof _chart.y()({}), "undefined", "returns number if value is undefined");
        assert.equal(_chart.y()({y: 1}), 1, "returns value if value is defined");
        let val = function(d){return 2;};
        _chart.y(val);
    ...
    t.test("size: ", function(assert) {
        ...
        var def = _chart.size();
        assert.equal(typeof def, "function", "returns default function");
        assert.equal(typeof _chart.size()({}), "number", "returns undefined if value is undefined");
    ...
    t.test("useVoronoi: ", function(assert) {
        assert.plan(2);
        let def = true;
        assert.equal(_chart.useVoronoi(), def, "returns default value");
        let val = false;
        _chart.useVoronoi(val);
        assert.equal(_chart.useVoronoi(), val, "returns set value");
        t.register(assert, type);
    });
    ...
    t.test("pointRadius: ", function(assert) {
        assert.plan(2);
        let def = 3;
        assert.equal(_chart.pointRadius(), def, "returns default value");
        let val = 5;
        _chart.pointRadius(val);
        assert.equal(_chart.pointRadius(), val, "returns set value");
        t.register(assert, type);
    });
    t.test("isArea: ", function(assert) {
        assert.plan(5);
        let def = _chart.isArea();
        assert.equal(typeof def, "function", "returns default function");
        assert.equal(_chart.isArea()({}), false, "returns false if data area undefined");
        assert.equal(_chart.isArea()({area: true}), true, "returns data defined area value");
        let val = function(){return false;};
        _chart.isArea(val);
        assert.equal(_chart.isArea(), val, "returns set function");
        assert.equal(_chart.isArea()(), false, "returns set function value");
        t.register(assert, type);
    });
    t.test("interpolate: ", function(assert) {
        assert.plan(2);
        let def = "linear";
        assert.equal(_chart.interpolate(), def, "returns default value");
        let val = "monotone";
        _chart.interpolate(val);
        assert.equal(_chart.interpolate(), val, "returns set value");
        t.register(assert, type);
    });
    ...
    t.test("defined: ", function(assert) {
        // defined = function(d, i) { return !isNaN(getY(d, i)) && getY(d, i) !== null; },
        assert.plan(6);
        let def = _chart.defined();
        assert.equal(typeof def, "function", "returns default function");
        assert.equal(_chart.defined()({}), false, "returns false if value is undefined");
        assert.equal(_chart.defined()({y: 1}), true, "returns true if value is defined");
        assert.equal(_chart.defined()({y: null}), false, "returns false if value is undefined");
        let val = function(){return false;};
        _chart.defined(val);
        assert.equal(_chart.defined(), val, "returns set function");
        assert.equal(_chart.defined()(), false, "returns set function value");
        t.register(assert, type);
    });
