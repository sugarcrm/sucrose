...
const data = require("../../files/pareto_data.json");
...
dreams.tests("INT: paretoChart -", function(t) {
  ...
  // Title tests
  t.test("title: do not render title by default", function(assert) {
    nightmare
      .chart(options)
      .config("default")
      .render(json)
  ...
  t.test("title: render title if showTitle set to 'false'", function(assert) {
    nightmare
      .chart(options)
      .config({
        "colorData": "default",
      ...
      .then(function(result) {
        assert.equal(result, "Test pareto chart");
        assert.end();
      })
      .catch(function(msg) { dreams.error(msg, assert, nightmare); });
  });
  ...
  t.test("color: palette is category20 by default", function(assert) {
    nightmare
      ...
      .render(json)
      .evaluate(function() {
        var result = getElement(".sc-multibar .sc-series-0");
        return {
  ...
  t.test("color: palette uses classnames if colorData set to 'class'", function(assert) {
    nightmare
      ...
      .then(function(result) {
        assert.plan(2);
        assert.equal(result.class, "sc-series sc-series-0 sc-fill00");
  ...
  t.test("color: palette controlled by color attribute of series Data", function(assert) {
    nightmare
      ...
      .evaluate(function(json) {
        ...
        d.data[0].color = "#000";
        render(d);
        result = getElement(".sc-multibar .sc-series-0");
        return {
          fill: result.getAttribute("fill"),
          class: result.getAttribute("class"),
        };
  ...
  t.skip("color: palette graduated on gray scale", function(assert) {
    nightmare
      .chart(options)
      .evaluate(function(json) {
        var result;
  ...
  t.test("dimensions: margins can be defined", function(assert) {
    nightmare
      ...
      .evaluate(function(json) {
        ...
        render(json);
        background = getElement(".sc-background-wrap rect");
  ...
  t.test("dimensions: default to container dimensions", function(assert) {
    nightmare
      ...
      .render(json)
      .evaluate(function() {
        var background = getElement(".sc-background-wrap rect");
        return {
  ...
  t.skip("tooltips: dispatching elementMouseover on chart triggers tooltip", function(assert) {
    nightmare
      ...
      .evaluate(function() {
        ...
        chart.mulitbar.dispatch.call("elementMouseover", this, {
          id: "asfd",
          key: "test",
          value: 100,
          count: 1,
  ...
  t.skip("tooltips: dispatching elementMouseout on chart destroys tooltip", function(assert) {
    nightmare
      ...
      .evaluate(function() {
        ...
        chart.mulitbar.dispatch.call("elementMouseout", this);
        return document.querySelector(".tooltip");
      })
      .then(function(result) {
        assert.equal(result, null);
  ...
  t.test("strings: chart should display custom strings", function(assert) {
    nightmare
      ...
      .evaluate(function(json) {
        ...
        render(d);
        result = {
          label: getInnerHtml(".sc-legend-wrap.sc-bar-legend .sc-series-0 text"),
          link: getInnerHtml(".sc-legend-wrap.sc-bar-legend .sc-menu-link"),
        };
        render(null);
  ...
  //     render(data);
  //     chart.multibar.dispatch.call("elementClick", this, {
  //           id: "asfd",
  //           key: "test",
  //           value: 100,
  //           count: 1,
